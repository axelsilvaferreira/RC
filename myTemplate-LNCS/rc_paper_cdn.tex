...
\begin{document}
\mainmatter
\title{Overview of Content-Delivery Networks Architecture Costs}
...
Due to its large distributed infrastructure, and high bandwidth on demand, CDNs are especially well suited to absorve a Denial of Service (DoS) attack.

=> % - Cheap to get going (pay as you go) no need for big investment at 		beguinning
	% - Capacity on Demand - Dominos friday noght
		-> falar do exemplo da pizza (usar outro exemplo)
=> % Exemplo (Âmbito de aplicação) Netflix [Tuga IO]
	exemplo akamai  
...
This paper's purpose is to provide organised information about different CDN architectures: the traditional large server infrastructure CDN, the Peer to Peer (P2P) CDN and a new Hybrid architecture which relies on both Servers and Peers. 
	...
	1 - overview das 3 arquitecturas principais
		Objetivo do paper - num único texto (organizar informação)
			- impacto das CDN na sociedade (âmbitos de aplicação)
			- dado o impacto social das CDN => é importante reduzir os custos associados ás CDN (infraestruturas) de forma a permitir que um numero maior de empresas (de pequena e media dimensão) ou particulares possam usufruir delas. =>impacto Redução da carga nas internet
...
\subsection{intro}
\subsection{ambito de aplicação}
...
\if
	Arquiteturas Propostas
		...
		-justificacao- baixa de precos e acessivel a + gente com 
\fi 
...
- a contribuição única é o paper focado nos custos gerais em função das diferentes arquiteturas

-Referir que este paper está limitado á prespetiva da bibliografia
